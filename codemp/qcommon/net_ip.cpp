...
static sockaddr_in socksRelayAddr;
...
static qboolean Sys_StringToSockaddr(const char* s, sockaddr_in* sadr)
{
	...
	else
	{
		...
		if ((h = gethostbyname(s)) == nullptr)
			return qfalse;
		sadr->sin_addr.s_addr = *reinterpret_cast<uint32_t*>(h->h_addr_list[0]);
	}
...
#endif
	ret = recvfrom(ip_socket, reinterpret_cast<char*>(net_message->data), net_message->maxsize, 0, reinterpret_cast<sockaddr*>(&from), &fromlen);
...
==================
*/
void Sys_SendPacket(const int length, const void* data, netadr_t to)
{
	int ret;
	sockaddr_in addr;
	...
	if (usingSocks && to.type == NA_IP)
	{
		...
		ret = sendto(ip_socket, socksBuf, length + 10, 0, reinterpret_cast<sockaddr*>(&socksRelayAddr), sizeof socksRelayAddr);
	}
	else
	{
		ret = sendto(ip_socket, static_cast<const char*>(data), length, 0, reinterpret_cast<sockaddr*>(&addr), sizeof addr);
	}
	if (ret == SOCKET_ERROR)
	{
		const int err = socketError;
...
static SOCKET NET_IPSocket(char* net_interface, const int port, int* err)
{
	...
	*err = 0;
	...
	Com_Printf("--------------------- Build Date 14/04/2026 ---------------------\n");// build date
	Com_Printf("---------------------------Build 05------------------------------\n");
	Com_Printf("-----------------------------------------------------------------\n");
	...
	if (setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<char*>(&i), sizeof i) == SOCKET_ERROR)
	{
...
static void NET_OpenSocks(const int port)
{
	...
	const hostent* h = gethostbyname(net_socksServer->string);
	if (h == nullptr)
	{
	...
	memset(&address, 0, sizeof address);
	address.sin_family = AF_INET;
	address.sin_addr.s_addr = *reinterpret_cast<uint32_t*>(h->h_addr_list[0]);
	...
	{
		// build the request
		const int ulen = strlen(net_socksUsername->string);
		const int plen = strlen(net_socksPassword->string);
...
static void NET_GetLocalAddress(void)
{
	...
	const hostent* hostInfo = gethostbyname(hostname);
	if (!hostInfo)
	{
		return;
	}
	...
	{
		const int ip = ntohl(*reinterpret_cast<uint32_t*>(p));
		localIP[numIP][0] = p[0];
		localIP[numIP][1] = p[1];
		localIP[numIP][2] = p[2];
		localIP[numIP][3] = p[3];
...
void NET_Config(qboolean enableNetworking)
{
	...
	const qboolean modified = NET_GetCvars();
