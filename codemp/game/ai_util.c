...
static int GetValueGroup(const char* buf, const char* group, char* outbuf)
{
	...
	const char* place = strstr(buf, group);
	...
	int startpoint = place - buf + strlen(group) + 1;
	int startletter = place - buf - 1;

	int failure = 0;

	while (buf[startpoint + 1] != '{' || buf[startletter] != '\n')
	{
		const char* placesecond = strstr(place + 1, group);
...
static int GetPairedValue(char* buf, const char* key, char* outbuf)
{
	...
	const char* place = strstr(buf, key);
	...
	//tab == 9
	int startpoint = place - buf + strlen(key);
	int startletter = place - buf - 1;
	...
	while (!found)
	{
		if (startletter == 0 || !buf[startletter] || buf[startletter] == '\0' || buf[startletter] == 9 || buf[
			startletter] == ' ' || buf[startletter] == '\n')
		{
...
int BotDoChat(bot_state_t* bs, const char* section, const int always)
{
	...
	int getthisline = Q_irand(0, lines + 1);
...
static int ReadChatGroups(const bot_state_t* bs, const char* buf)
{
	...
	int cgbplace = cgroupbegin - buf + 1;
