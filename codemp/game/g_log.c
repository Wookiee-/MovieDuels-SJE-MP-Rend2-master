...
int weaponFromMOD[MOD_MAX] =
{
	...
	WP_Z6_ROTARY_CANNON,				//MOD_Z6_ROTARY_CANNON,
	...
	WP_NONE, //MOD_VEH_EXPLOSION,
	WP_NONE, //MOD_SEEKER,
	WP_NONE, //MOD_ELECTROCUTE,
	WP_NONE, //MOD_SPECTATE,
	WP_NONE, //MOD_HEADSHOT,
	WP_NONE, //MOD_BODYSHOT,
	WP_NONE, //MOD_FORCE_LIGHTNING,
	WP_NONE, //MOD_BURNING,
	WP_NONE, //MOD_GAS,
};
...
void QDECL G_LogWeaponFire(const int client, const int weaponid)
{
	...
	const int dur = level.time - G_WeaponLogLastTime[client];
...
qboolean CalculateEfficiency(const gentity_t* ent, int* efficiency)
{
	...
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse)
			continue;
		const int nShotsFired = player->client->accuracy_shots; //player->client->ps.persistant[PERS_ACCURACY_SHOTS];
		const int nShotsHit = player->client->accuracy_hits; //player->client->ps.persistant[PERS_ACCURACY_HITS];
		const float fAccuracyRatio = (float)nShotsHit / (float)nShotsFired;
	...
	{
		const int tempEff = (int)(100 * fBestRatio);
		if (tempEff > 50)
		{
			*efficiency = tempEff;
			return qtrue;
...
qboolean CalculateSharpshooter(const gentity_t* ent, int* frags)
{
	...
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse)
			continue;
		const int nKills = G_WeaponLogKills[i][MOD_DISRUPTOR_SNIPER];
...
static qboolean CalculateTactician(const gentity_t* ent, int* kills)
{
	...
	for (person = 0; person < sv_maxclients.integer; person++)
	{
		const gentity_t* player = g_entities + person;
		if (!player->inuse) continue;

		int nKills = 0; // This Persons's Kills
		for (weapon = 0; weapon < WP_NUM_WEAPONS; weapon++)
...
qboolean CalculateDemolitionist(const gentity_t* ent, int* kills)
{
	...
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		int nKills = G_WeaponLogKills[i][MOD_THERMAL];
...
qboolean CalculateTeamMVP(const gentity_t* ent)
{
	...
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse || player->client->ps.persistant[PERS_TEAM] != team)
			continue;
		const int nScore = player->client->ps.persistant[PERS_SCORE];
...
qboolean CalculateTeamDefender(const gentity_t* ent)
{
	...
	*/
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse || player->client->ps.persistant[PERS_TEAM] != team)
			continue;
		const int nScore = player->client->pers.teamState.basedefense;
		if (nScore > nHighestScore)
		{
			nHighestScore = nScore;
			nBestPlayer = i;
...
qboolean CalculateTeamCarrier(const gentity_t* ent)
{
	...
	*/
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse || player->client->ps.persistant[PERS_TEAM] != team)
			continue;
		const int nScore = player->client->pers.teamState.captures;
		if (nScore > nHighestScore)
		{
			nHighestScore = nScore;
			nBestPlayer = i;
...
qboolean CalculateTeamInterceptor(const gentity_t* ent)
{
	...
	*/
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse || player->client->ps.persistant[PERS_TEAM] != team)
			continue;
		int nScore = player->client->pers.teamState.flagrecovery;
...
qboolean CalculateTeamRedShirt(const gentity_t* ent)
{
	...
	*/
	for (int i = 0; i < sv_maxclients.integer; i++)
	{
		...
		if (!player->inuse || player->client->ps.persistant[PERS_TEAM] != team)
			continue;
		int nScore = player->client->ps.persistant[PERS_KILLED];
...
typedef enum
{
	...
	// Accuracy
	AWARD_SHARPSHOOTER,
	// Most compression rifle frags
	AWARD_UNTOUCHABLE,
	// Perfect (no deaths)
	AWARD_LOGISTICS,
	// Most pickups
	AWARD_TACTICIAN,
	// Kills with all weapons
	AWARD_DEMOLITIONIST,
	// Most explosive damage kills
	AWARD_STREAK,
	// Ace/Expert/Master/Champion
	AWARD_TEAM,
	// MVP/Defender/Warrior/Carrier/Interceptor/Bravery
	AWARD_SECTION31,
	// All-around god
	AWARD_MAX
} awardType_t;
...
typedef enum
{
	...
	// most overall points
	TEAM_DEFENDER,
	// killed the most baddies near your flag
	TEAM_WARRIOR,
	// most frags
	TEAM_CARRIER,
	// infected the most people with plague
	TEAM_INTERCEPTOR,
	// returned your own flag the most
	TEAM_BRAVERY,
	// Red Shirt Award (tm). you died more than anybody.
	TEAM_MAX
} teamAward_e;
