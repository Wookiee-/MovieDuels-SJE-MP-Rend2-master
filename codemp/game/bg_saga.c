...
stringID_table_t HoldableTable[] =
{
	...
	ENUM2STRING(HI_FLAMETHROWER),
	ENUM2STRING(HI_SWOOP),
	ENUM2STRING(HI_DROIDEKA),
	ENUM2STRING(HI_SPHERESHIELD),

	ENUM2STRING(HI_GRAPPLE),
...
stringID_table_t PowerupTable[] =
{
	...
	ENUM2STRING(PW_INVINCIBLE),
	ENUM2STRING(PW_GALAK_SHIELD),
	ENUM2STRING(PW_FORCE_PUSH),
	ENUM2STRING(PW_FORCE_PUSH_RHAND),
	ENUM2STRING(PW_FORCE_PROJECTILE),
	ENUM2STRING(PW_SPHERESHIELDED),
			...
			else
			{
				...
				if (isGroup && !Q_stricmp(checkGroup, group))
				{
					...
					if (buf[i])
					{
						...
						while ((buf[i] != '}' || parse_groups) && buf[i])
						{
							if (buf[i] == '{')
							{
				...
				else
				{
					...
					while (buf[i] && (buf[i] != '}' || parse_groups))
					{
						if (buf[i] == '{')
						{
							parse_groups++;
			...
			if (checkPower[0])
			{
				...
				while (FPTable[k].id != -1 && FPTable[k].name[0])
				{
					if (!Q_stricmp(checkPower, FPTable[k].name))
					{
						...
						siegeClass->forcePowerLevels[k] = parsed_level;
						break;
					}
					k++;
				}
...
static void BG_SiegeParseClassFile(const char* filename, siegeClassDesc_t* descBuffer)
{
	...
	if (BG_SiegeGetPairedValue(classInfo, "classflags", parseBuf))
	{
		bgSiegeClasses[bgNumSiegeClasses].classflags = BG_SiegeTranslateGenericTable(
			parseBuf, bgSiegeClassFlagNames, qtrue);
	}
	else
	{
		//fine, we'll 0 it.
...
#endif
		{
			const int titleLength = strlen(parseBuf);
			for (i = 0; i < SPC_MAX; i++)
			{
				// Back up
				const int arrayTitleLength = strlen(classTitles[i]);
				...
				if (strcmp(hold_buf, classTitles[i]) == 0)
				{
...
void BG_SiegeLoadClasses(siegeClassDesc_t* descBuffer)
{
	...
	const int numFiles = trap->FS_GetFileList("ext_data/Siege/Classes", ".scl", filelist, sizeof filelist);
...
static void BG_SiegeParseTeamFile(const char* filename)
{
	...
	if (BG_SiegeGetValueGroup(teamInfo, "Classes", teamInfo))
	{
		...
		while (success && i < MAX_SIEGE_CLASSES)
		{
			...
			bgSiegeTeams[bgNumSiegeTeams].classes[bgSiegeTeams[bgNumSiegeTeams].numClasses] =
...
void BG_SiegeLoadTeams(void)
{
	...
	const int numFiles = trap->FS_GetFileList("ext_data/Siege/Teams", ".team", filelist, sizeof filelist);
...
void BG_PrecacheSabersForSiegeTeam(const int team)
{
	...
	const siegeTeam_t* t = BG_SiegeFindThemeForTeam(team);
...
qboolean BG_SiegeCheckClassLegality(const int team, char* classname)
{
	...
	while (i < (*team_ptr)->numClasses)
	{
		if (!Q_stricmp(classname, (*team_ptr)->classes[i]->name))
		{
	...
	strcpy(classname, (*team_ptr)->classes[0]->name);
...
void BG_SiegeSetTeamTheme(const int team, const char* themeName)
{
	...
	*team_ptr = BG_SiegeFindTeamForTheme(themeName);
}
