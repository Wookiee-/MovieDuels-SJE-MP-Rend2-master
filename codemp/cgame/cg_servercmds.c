...
static void CG_ParseScores(void)
{
	...
	for (int i = 0; i < readScores; i++)
	{
		...
		const int powerups = atoi(CG_Argv(i * SCORE_OFFSET + 9));
...
void CG_SetConfigValues(void)
{
	...
	if (cgs.gametype == GT_CTF || cgs.gametype == GT_CTY)
	{
		...
		const int blueflag_id = s[1] - '0';

		// fix: proper flag statuses mapping for dropped flag
		if (redflag_id >= 0 && redflag_id < ARRAY_LEN(ctfFlagStatusRemap))
			cgs.redflag = ctfFlagStatusRemap[redflag_id];

		if (blueflag_id >= 0 && blueflag_id < ARRAY_LEN(ctfFlagStatusRemap))
			cgs.blueflag = ctfFlagStatusRemap[blueflag_id];
	}
	...
	const char* str = CG_ConfigString(CS_CLIENT_DUELISTS);
...
static void CG_RegisterCustomSounds(clientInfo_t* ci, const int setType, const char* psDir)
{
	...
	for (int i = 0; i < iTableEntries; i++)
	{
		...
		s++;
		sfxHandle_t hSFX = trap->S_RegisterSound(va("sound/chars/%s/misc/%s", psDir, s));
...
void CG_HandleNPCSounds(const centity_t* cent)
{
	...
	//jedi
	if (cent->currentState.csSounds_Custom)
	{
		const char* s = CG_ConfigString(CS_SOUNDS + cent->currentState.csSounds_Custom);
	...
	{
		memset(&cent->npcClient->calloutSounds, 0, sizeof cent->npcClient->calloutSounds);
	}
}
...
static void CG_BodyQueueCopy(centity_t* cent, const int clientNum, const int knownWeapon)
{
...
static void CG_NewForceRank_f(void)
{
	...
	const int newRank = atoi(CG_Argv(1));
	const int doMenu = atoi(CG_Argv(2));
	const int setTeam = atoi(CG_Argv(3));
...
static void CG_RestoreClientGhoul_f(void)
{
	...
	centity_t* clent = &cg_entities[indexNum];
